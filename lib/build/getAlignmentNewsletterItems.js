...
let getNum = (emailField) =>{
    let anText = flush(emailField)
    let anNumber = anText.replace("AN #", "")
    return anNumber
}
let getLink = (emailField) =>{
    let href = emailField ? emailField.split(`href="`)[1] : null
    let link = href ? href.split(`"`)[0] : "N/A"
    return link
}
...
let getProperty = (property, item) => {
    switch (property) {
        ...
        case "anNewsletterNumber":
            return getNum(item["Email"])
        case "anNewsletterLink":
            return getLink(item["Email"])
        default:
            return "";
    }
}
...
export default async function getAlignmentNewsletterItems() {
    ...
    let alignmentNewsletterFormatted = alignmentNewsletterJSON.map(item => {
        return ({
            ...
            anNewsletterNumber:getProperty("anNewsletterNumber", item),
            anNewsletterLink:getProperty("anNewsletterLink", item),
            jeremyBlurb: null,
            conferenceName: null
        })
    })
