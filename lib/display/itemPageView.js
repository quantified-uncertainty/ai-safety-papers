...
function tableOfPublications({publications, publicationsPage, title}){
  if(publications.length > 0){
    ...
    return(<div>Not parsed, but see <a href={publicationsPage} target="_blank" className="underline text-blue-600 hover:text-blue-800">{publicationsPage}</a></div>)
  }else{
...
export default function ItemPageView({
  ...
  return (
    <div key={id} className="container mx-auto pt-10 max-w-5xl">
      ...
      {tableOfPublications({publications, publicationsPage, title})}
      <br/>
      &nbsp;
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {forecasts.length > 0 ? displayForecasts(forecasts) : "∅"}
      </div>      
      <br/>
      &nbsp;
